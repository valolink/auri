...
export const ajaxRequest = async function (action = 'pdf_report') {
  ...
  formData.append('addressFromApi', output.addressFromApi || '')
  ...
  for (let i = 0; i < outputs.length; i++) {
    ...
    formData.append(
      ...
      output[outputs[i]].paybackYears?.toFixed(1) || '0',
    )
    formData.append(
    ...
    formData.append(
      ...
      (roundToSignificantFigures(output[outputs[i]].lcoeSntPerKwh) || 0).toString(),
    )
    formData.append(
